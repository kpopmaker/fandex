...
VERSION = "fantagio_music_identity_roster_adapter_v1"
DEFAULT_URL = "https://www.fantagio.kr/groups/"
EXPECTED_ARTISTS = [
    "LUN8",
    "ZOONIZINI",
    "LEECHANGSUB",
    ...
    return [
        {
            ...
            "evidence": [
                {
                    "label": "Fantagio official music identity roster",
                    "url": source_url,
                }
            ],
        }
...
def build_snapshot(rows: list[dict], source_url: str, observed_at: str) -> dict:
    return {
        "version": VERSION,
        "source": {
            "id": "fantagio-official-music-identity-roster",
            "type": "agency_roster",
            "name": "Fantagio Official Music Identity Roster",
...
def load_verified_fallback(path: Path) -> dict:
    ...
    if not isinstance(source, dict):
        raise RuntimeError("Fantagio fallback snapshot source object required")
    if source.get("id") != "fantagio-official-music-identity-roster":
        raise RuntimeError("Fantagio fallback snapshot source id mismatch")
    if source.get("type") != "agency_roster":
        raise RuntimeError("Fantagio fallback snapshot source type mismatch")
    candidates = snapshot.get("candidates")
    if not isinstance(candidates, list) or len(candidates) != len(EXPECTED_ARTISTS):
        raise RuntimeError("Fantagio fallback snapshot expected roster required")
...
def main() -> None:
    ...
    if len(rows) == len(EXPECTED_ARTISTS):
        ...
        raise RuntimeError(
            f"Fantagio roster adapter expected {len(EXPECTED_ARTISTS)} identities, got {len(rows)}"
        )
