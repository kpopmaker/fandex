...
VERSION = "naver_v3_expand_full_then_publish_safe_v2_auto_collect_yes"
...
def main():
    ...
    backup_dir = Path(f"naver_v3_backup_before_full_expand_auto_yes_{timestamp}")
    original_files = backup_naver_latest_files(backup_dir)

    print()
    print("Naver v3 expand full then publish safe runner v2")
    print("=" * 70)
    print(f"version: {VERSION}")
    print(f"expected artist count: {len(expected_artists)}")
    print(f"before naver ranking count: {len(before_artists)}")
    print(f"backup dir: {backup_dir}")
    print("원본 데이터 수집 질문에는 자동으로 y를 입력합니다.")
    print("주의: website public/data는 건드리지 않습니다.")
    print()

    full_result = run_capture(FULL_PIPELINE_SCRIPT, input_text="y\n")
    ...
    lines.append("fullPipelineInput: y")
    ...
    lines.append("-" * 70)
    lines.append(full_result["stdout"][-6000:] if str(full_result["stdout"]).strip() else "(empty)")

    lines.append("")
    lines.append("FULL PIPELINE STDERR tail")
    lines.append("-" * 70)
    lines.append(full_result["stderr"][-6000:] if str(full_result["stderr"]).strip() else "(empty)")

    lines.append("")
    lines.append("PUBLISH STDOUT tail")
    lines.append("-" * 70)
    lines.append(str(publish_result["stdout"])[-6000:] if str(publish_result["stdout"]).strip() else "(empty)")

    lines.append("")
    lines.append("PUBLISH STDERR tail")
    lines.append("-" * 70)
    lines.append(str(publish_result["stderr"])[-6000:] if str(publish_result["stderr"]).strip() else "(empty)")
    ...
    print()
    print("=" * 70)
    print("Naver v3 expand full then publish safe runner v2 완료")
    print("=" * 70)
