...
VERSION = "naver_v3_expand_full_then_publish_safe_v1"
...
def main():
    ...
    backup_dir = Path(f"naver_v3_backup_before_full_expand_{timestamp}")
    original_files = backup_naver_latest_files(backup_dir)

    print()
    print("Naver v3 expand full then publish safe runner")
    print("=" * 70)
    ...
    full_result = run_capture(FULL_PIPELINE_SCRIPT)
    ...
    lines.append("-" * 70)
    lines.append(full_result["stdout"][-5000:] if str(full_result["stdout"]).strip() else "(empty)")

    lines.append("")
    lines.append("FULL PIPELINE STDERR tail")
    lines.append("-" * 70)
    lines.append(full_result["stderr"][-5000:] if str(full_result["stderr"]).strip() else "(empty)")

    lines.append("")
    lines.append("PUBLISH STDOUT tail")
    lines.append("-" * 70)
    lines.append(str(publish_result["stdout"])[-5000:] if str(publish_result["stdout"]).strip() else "(empty)")

    lines.append("")
    lines.append("PUBLISH STDERR tail")
    lines.append("-" * 70)
    lines.append(str(publish_result["stderr"])[-5000:] if str(publish_result["stderr"]).strip() else "(empty)")
    ...
    print()
    print("=" * 70)
    print("Naver v3 expand full then publish safe runner 완료")
    print("=" * 70)
