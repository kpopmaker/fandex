...
VERSION = "patch_daily_runner_lastfm_preview_v1"
...
BACKUP = Path(
    "run_fandex_daily_python_only_before_lastfm_preview_v1.bat"
)
...
def main():
    ...
    print()
    print("FANDEX patch daily runner Last.fm preview v1")
    print("=" * 76)
    ...
    if OLD_BLOCK not in original:
        raise SystemExit(
            "ERROR: Last.fm delta/health anchor를 "
            "찾지 못했습니다."
        )
    ...
    if core_anchor not in patched:
        raise SystemExit(
            "ERROR: Last.fm core file anchor 없음"
        )
    ...
    print()
    print("daily runner 1/9~9/9: READY")
    print("Last.fm score preview: READY")
    print("Last.fm impact preview: READY")
    ...
    if not apply_mode:
        ...
        print("적용 명령:")
        print(
            "py patch_daily_runner_lastfm_preview_v1.py --apply"
        )
        return
