...
VERSION = "fandex_publish_all_v1_youtube_master_v5"
...
STEPS = [
    ...
    {
        "step": "master_score_v5",
        "label": "3. FANDEX master v5 무상한 누적 점수 생성",
        "script": "fandex_master_score_v5.py",
    },
]
...
def verify_outputs():
    ...
    if os.path.exists(master_path):
        try:
            ...
            if version != "fandex_master_v5_uncapped_cumulative_points":
    ...
    youtube_path = "fandex_youtube_ranking_v2_latest.json"

    if os.path.exists(youtube_path):
        try:
            data = read_json(youtube_path)
            ranking = data.get("ranking", [])

            if not ranking:
                problems.append("YouTube v2 ranking 데이터 없음")

        except Exception as e:
            problems.append(f"YouTube v2 ranking JSON 읽기 실패: {e}")
...
def print_final_preview():
    ...
    for item in ranking:
        ...
        naver = source_points.get("naver", {})
        youtube = source_points.get("youtube", {})
        youtube_signal = youtube.get("coreSignal", "")
        ...
        print(
            ...
            f"{' / 유튜브 핵심: ' + youtube_signal if youtube_signal else ''})"
        )
...
def main():
    ...
    log_file = f"fandex_publish_all_v1_log_{now}.csv"
    logs = []

    print()
    print("FANDEX publish all v1 시작")
    print("=" * 60)
    print("실행 순서: YouTube 수집 → YouTube v2 → FANDEX master v5")
    print("master v5는 비율 합산이 아니라 무상한 누적 점수제입니다.")
    print("=" * 60)
    ...
    print()
    print("=" * 60)
    print("FANDEX publish all v1 완료")
    print("=" * 60)
