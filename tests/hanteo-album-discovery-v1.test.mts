    ...
    list: [
      {
        ...
        rankDiff: 0,
        targetIdx: '900562834',
        targetImg: '/album/900562834_s150.jpg',
        ...
        detail: {
          ...
          entertainment: '웨이크원',
          artistIdx: 76154,
          artistName: '알파드라이브원(ALPHA DRIVE ONE)',
...
test('Hanteo current Daily Weekly Monthly plans are deterministic GET routes with required limit', () => {
  ...
  for (const plan of [day, week, month]) {
    ...
    assert.equal(plan.historicalSelectorState, 'pending');
...
test('Hanteo historical request remains fail-closed until selector is directly qualified', () => {
  assert.throws(
...
test('Hanteo code 602 missing-limit response is a provider request error, not empty chart data', () => {
  const decoded = decodeHanteoAlbumResponse({
    code: 602,
    message: "Required int parameter 'limit' is not present",
    resultData: null,
  });
...
test('Hanteo salesVolume is copies while value remains Album Index', () => {
  const decoded = decodeHanteoAlbumResponse(weeklyResponse);
  const qualified = qualifyHanteoAlbumRow(decoded.rows[0], {
...
test('Hanteo row qualification fails closed when salesVolume or detail is missing', () => {
  ...
  assert.throws(
    () => qualifyHanteoAlbumRow({ ...base, detail: { ...base.detail, salesVolume: undefined } }, { quantityEvidenceId: 'evidence-1' }),
    /sales-volume-invalid/,
  );
  assert.throws(
    () => qualifyHanteoAlbumRow({ ...base, detail: undefined }, { quantityEvidenceId: 'evidence-1' }),
    /sales-volume-invalid/,
  );
});
...
test('Hanteo quantity semantics require explicit evidence and never fall back to rank or Album Index', () => {
  ...
  const withoutSales = {
    ...
    rank: 1139747,
    value: 1139747,
    detail: { ...(row.detail as Record<string, unknown>), salesVolume: undefined },
  };
