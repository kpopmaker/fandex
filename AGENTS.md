...
5. Do not call GitHub merge or enable-auto-merge tools for version PRs. The repository workflow `.github/workflows/codex-version-pr-auto-merge.yml` can perform an exact-base/head squash merge only after all checks pass, at least one current trusted non-author APPROVED review targets the exact head SHA, no current trusted reviewer has an active CHANGES_REQUESTED decision on that SHA, and the exact `production-merge-approved` label is present. It must not leave a persistent GitHub auto-merge reservation.
