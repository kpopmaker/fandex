...
1. The PR is open, same-repository, owner-authored, still targets the `main` ref, uses a `vN-*` head, and is not Draft. Closing, retargeting, or converting the PR back to Draft creates a new concurrency event that cancels the prior run.
...
7. During Vercel polling and immediately before merge, the workflow rechecks open state, the exact `main` base-ref name, base/head SHAs, Draft state, label, and current trusted review state.
...
9. Only the final merge job receives `contents: write`; pull-request metadata remains read-only in every job.
