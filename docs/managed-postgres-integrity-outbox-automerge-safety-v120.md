...
1. The PR is open, same-repository, authored by the repository owner's human GitHub `User` account, still targets the `main` ref, uses a `vN-*` head, and is not Draft. Closing, retargeting, changing the head, or converting the PR back to Draft creates a concurrency event that cancels the prior run.
2. A top-level PR comment from that same owner has `author_association: OWNER` and exactly records the current base and head in the three-line `FANDEX_PRODUCTION_MERGE_ATTESTATION v1` format. Bot, other-account, malformed, extra-text, and stale-SHA comments cannot authorize a merge.
3. The exact `production-merge-approved` label is present as a separate final authorization and is added only after the current owner attestation exists.
4. Comment pagination is flattened before evaluation. Comment create, edit, and delete events share the PR concurrency key, so changing or removing the attestation cancels an in-progress run.
...
7. During Vercel polling and immediately before merge, the workflow rechecks open state, the exact `main` base-ref name, base/head SHAs, Draft state, label, and current exact owner attestation.
8. The workflow composes a merge commit whose two parents are the exact authorized base and head, confirms remote `main` still equals the authorized base, and updates `main` with an exact expected-base `--force-with-lease`. Any concurrent movement of `main`, including movement to the PR head or one of its ancestors, makes the lease fail closed.
9. Only the final merge job receives `contents: write`; PR and issue-comment metadata remain read-only in every job.

Ready-for-review, owner attestation, and merge authorization are intentionally separate states. Removing Draft status alone cannot activate the merge path. The attestation is a solo-maintainer accountability record rather than independent peer review. The workflow does not enable a persistent GitHub auto-merge reservation and prohibits unconditional force.

## Validation boundary

The v120 test set covers derived digest tampering, closure-lineage substitution, invalid version transitions, versioned idempotency, explicit legacy-v116 classification, single-statement pre-state reads, requested-field pre-state/CAS/postcondition enforcement, bounded final-attempt lease expiry, open-state/base-ref cancellation, least-privilege workflow permissions, exact-base/head validation and exact-base lease behavior, solo-owner attestation negatives, explicit labeling, and repository instructions. An exact legacy v116 state now fails before writes with `legacy_v116_state_requires_fresh_branch`; validating the new contract requires a fresh staging branch under separate DB authorization. Database integration remains outside this change because DB work was not authorized.
